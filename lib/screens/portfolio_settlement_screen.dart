...
class _PortfolioSettlementScreenState extends State<PortfolioSettlementScreen> {
  ...
  Widget _itemRow(BuildContext context, Portfolio pf,
    ...
    return ContributionRow(
      ...
      endValue: c.endValue,
      share: sumAbs > 0 ? c.itemAbsoluteReturn.abs() / sumAbs : 0.0,
  ...
  Future<void> _saveImage() async {
    ...
    try {
      final bytes = await captureWidget(context, _buildCapture());
      if (bytes == null) {
  ...
  double get _sumAbs =>
      (_current?.contributions ?? const [])
          .fold(0.0, (s, c) => s + c.itemAbsoluteReturn.abs());
      ...
      rows: [
        for (final c in (r?.contributions ?? const []).take(8))
          CaptureRow(
            ...
            startValue: c.startValue,
            endValue: c.endValue,
            share: _sumAbs > 0 ? c.itemAbsoluteReturn.abs() / _sumAbs : 0.0,
            contribution: c.contribution,
          ),
      ],
    );
