...
class _PortfolioDetailScreenState extends State<PortfolioDetailScreen> {
  ...
  Future<void> _doRefresh(Portfolio pf) async {
    ...
    if (pf.priceAuto) {
      final stockItems =
          pf.items.where((i) => !i.isCash && i.ticker.isNotEmpty).toList();
    ...
    if (errors.isEmpty) _showToast('$successCount건 업데이트 완료', Colors.green);
      ...
      _showToast('$successCount건 성공, ${errors.length}건 실패', Colors.orange);
          ...
          TextButton(
            onPressed: () {
              ...
              context
                  ...
                  .applyRebalancing(pf.id, results, rb.cash);
              _investController.text =
                  rb.cash > 0 ? rb.cash.toStringAsFixed(0) : '';
              Navigator.pop(context);
            },
            child: const Text('적용'),
          ),
  ...
  void _openGraph(Portfolio pf) {
    Navigator.push(
      ...
      MaterialPageRoute(
          builder: (_) =>
              PortfolioGraphScreen(portfolioId: pf.id)),
    );
  }
      ...
      builder: (context, provider, _) {
        ...
        if (pf == null) {
          return const Scaffold(
              body: Center(child: Text('포트폴리오를 찾을 수 없습니다')));
        }

        if (_investController.text.isEmpty && pf.additionalInvestment > 0) {
          _investController.text =
              pf.additionalInvestment.toStringAsFixed(0);
        }
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        TextField(
                          ...
                          onChanged: (v) {
                            provider.setAdditionalInvestment(
                                pf.id, double.tryParse(v) ?? 0);
                          },
                        ),
                        const SizedBox(height: 10),
                        Row(children: [
                          _infoBox(
                              ...
                              rb != null
                                  ? _fmtAmount(
                                      rb.total - pf.additionalInvestment,
                                      pf.currency,
                                      pf.compactAmount)
                                  : '—'),
                          ...
                          _infoBox(
                              ...
                              '리밸런싱 기준',
                              rb != null
                                  ? _fmtAmount(rb.total, pf.currency,
                                      pf.compactAmount)
                                  : '—'),
                          const SizedBox(width: 8),
                          _infoBox(
                        ...
                        if (rb != null && commOn && rb.commission > 0)
                          Padding(
                            ...
                            child: Text(
                                '예상 수수료: ${_fmt(rb.commission, pf.currency)}',
                                style: TextStyle(
                                    fontSize: 12,
                        ...
                        Padding(
                          ...
                          child: Row(
                            ...
                            children: [
                              Text(
                                  '환율: 1 USD = ₩${pf.exchangeRate.toStringAsFixed(0)}',
                                  style: TextStyle(
                                      fontSize: 12,
                                      color: context.textHint)),
                ...
                if (pf.items.isNotEmpty && (weightSum - 100).abs() > 0.01)
                  Container(
                    ...
                    child: Row(children: [
                      ...
                      const SizedBox(width: 8),
                      Text(
                          '목표 비중 합계: ${_pct(weightSum)} (100%가 아닙니다)',
                          style: TextStyle(
                              fontSize: 13,
                      ...
                      : _editMode
                          ? ReorderableListView.builder(
                              ...
                              onReorder: (o, n) {
                                if (n > o) n--;
                                final list =
                                    List<PortfolioItem>.from(pf.items);
                                final item = list.removeAt(o);
                                list.insert(n, item);
                                provider.reorderItems(pf.id, list);
                              },
                              itemBuilder: (ctx, idx) =>
                                  _buildItemCard(context, pf, pf.items[idx],
                                      rb,
  ...
  Widget _marketBadge(BuildContext context, PortfolioItem item) {
    if (item.isCash) {
      return _badge(context, '현금', const Color(0xFF65A30D));
    }
    if (item.market == 'US') {
