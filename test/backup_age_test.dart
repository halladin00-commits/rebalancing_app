...
void main() {
  ...
  test('한 번도 안 했으면 없음 — 그리고 처음부터 눈에 띈다', () {
    final a = elapsedSince(null, now: now);
    expect(a.unit, ElapsedUnit.never);
  ...
  test('2일부터 29일까지는 일 단위', () {
    for (final d in [2, 15, 29]) {
      final a = elapsedSince(ago(d), now: now);
      expect(a.unit, ElapsedUnit.days, reason: '$d일');
      expect(a.count, d);
    }
  });
  ...
  test('30일이 되는 순간부터 경고색으로 바뀐다', () {
    expect(elapsedSince(ago(29), now: now).stale, isFalse);
    expect(elapsedSince(ago(30), now: now).stale, isTrue);
    expect(elapsedSince(ago(400), now: now).stale, isTrue);
  });

  test('시계가 뒤로 간 경우에도 "오늘"로 본다 — 음수 일수가 새어나가지 않는다', () {
    final a = elapsedSince(now.add(const Duration(days: 3)), now: now);
