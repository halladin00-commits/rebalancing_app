...
void main() {
  ...
  testWidgets('크림 바탕용 판은 딥그린으로 그린다', (tester) async {
    // 민트(#8FE7B0)를 크림 위에 그대로 쓰면 대비가 1.39:1이라 형태가
    // 안 읽힌다. 색을 뒤집어야 한다.
    const cream = Color(0xFFFBF8F1);
    final key = GlobalKey();
    await tester.pumpWidget(MaterialApp(
      home: Scaffold(
        ...
        body: Center(
          child: RepaintBoundary(
            ...
            child: Container(
              ...
              height: 240,
              color: cream,
              child: const CustomPaint(painter: TargetMarkPainter.onLight()),
            ),
          ),
        ),
      ),
    ...
    await tester.runAsync(() async {
      ...
      final image = await board.toImage(pixelRatio: 1);
      px = image.width;
      d = (await image.toByteData(format: ui.ImageByteFormat.rawRgba))!;
    });
    ...
    const rr =
        (TargetMarkPainter.ringOuter + TargetMarkPainter.ringInner) / 2;
    (double, double) polar(double deg) {
      final a = deg * math.pi / 180;
      return (0.5 + rr * math.sin(a), 0.5 - rr * math.cos(a));
    }

    // 아래·왼쪽 조각과 가운데 점은 딥그린이라 **어둡다**.
    for (final deg in const [180.0, 300.0]) {
      final (fx, fy) = polar(deg);
      final (r, g, b) = at(fx, fy);
      expect(r + g + b, lessThan(350),
          reason: '$deg° 조각이 크림 위에서 안 보인다');
    }
    final (cr, cg, cb) = at(0.5, 0.5);
    expect(cr + cg + cb, lessThan(350), reason: '가운데 점이 안 보인다');

    // 오른쪽 조각은 연하지만 바탕(크림)과는 달라야 한다.
    final (bx, by) = polar(60);
    final (ar, ag, ab) = at(bx, by);
    expect(ar + ag + ab, lessThan(620),
        reason: '오른쪽 조각이 크림 바탕과 구분이 안 된다');
  });
}
